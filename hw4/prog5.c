...
HashdataType *allhash() {
        ...
        if ((hashtable = malloc(sizeof(HashdataType) * HASHTAB_SIZE)) == NULL) { perror ("malloc hashdata"); exit(4); }
        ...
        do {
                ...
                for(i = 0; i < NUMBUCKETS; ++i) {
                        ...
                        if (htab[hindex].count == 0) {
                                ...
                                ++htab[hindex].count;
                                break;
                        } else if (strncmp(htab[hindex].word, p1, wsize ) == 0) {
                                        ++htab[hindex].count;
                                        break;
                        } else ++htab[hindex].colis;
                }
